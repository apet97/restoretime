...
| IC-4 | **R11 addon-token success path PROVED**: with the captured addon token, `users.list` (10 users, all ACTIVE), `projects.list` (36 projects), `timeEntries.createForUser` **for another user** (`69bda6b317a0c5babe34b50d` ≠ installer) → **201** with the target `userId`, `timeEntries.get`, `timeEntries.delete` all succeeded (probe `tools/install-capture/probe-addon-token.mjs`) | PROVED |
