...
export const PRODUCTION_API_URL = "https://api.clockify.me/api";
...
export function checkLiveEnv(): EnvCheck {
  ...
  return { blocked: false, env: { apiKey: process.env.CK_LIVE_API_KEY!, workspaceId: process.env.CK_LIVE_WS! } };
}
...
export function describeIfAuthRejected(err: unknown): string | undefined {
  if (err instanceof ClockifyApiError && err.statusCode === 401) {
    return `Clockify rejected CK_LIVE_API_KEY as an addon token (401 code ${clockifyErrorCode(err) ?? "unknown"}) — this is a real R11 finding, not a suite defect.`;
  }
  return undefined;
}
...
export function buildLiveRestClient(env: LiveEnv) {
  return buildClockifyClient({ apiUrl: PRODUCTION_API_URL, authToken: env.apiKey });
}
...
/** Boots the real `createServer()` against a throwaway local SQLite file, installs it (test-signed
 * lifecycle event) with the installation's Clockify credentials set to the REAL production API and
 * the REAL `CK_LIVE_API_KEY` — from this point on, every Clockify call the booted server makes for
 * this workspace goes out over the real network. */
